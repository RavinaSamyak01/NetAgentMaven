		...
		for (int row1 = 1; row1 < rowNum; row1++) {
			...
			if (ServiceID.contains("LOC") || ServiceID.contains("SD")) {
				...
				if (Orderstage.equalsIgnoreCase("Confirm Pu Alert")) {
					...
					try {
						...
						try {
							...
							if (ServiceID.equalsIgnoreCase("LOC")) {
								...
								js.executeScript("arguments[0].scrollIntoView();", Del);
					...
					WebElement PUSection = Driver.findElement(By.xpath("//*[@class=\"well-sm Pickupdata\"]"));
					js.executeScript("arguments[0].scrollIntoView();", PUSection);
					...
					try {
						...
						if (ServiceID.equalsIgnoreCase("LOC")) {
							...
							PUSection = Driver.findElement(By.xpath("//*[@class=\"well-sm Pickupdata\"]"));
							js.executeScript("arguments[0].scrollIntoView();", PUSection);
			...
			/*
			 * } catch (Exception NoData1) { try {
			 * wait.until(ExpectedConditions.visibilityOfElementLocated(By.className(
			 * "dx-datagrid-nodata"))); WebElement NoData =
			 * Driver.findElement(By.className("dx-datagrid-nodata")); if
			 * (NoData.isDisplayed()) {
			 * logger.info("Job is not exist with the search parameters");
			 * 
			 * } } catch (Exception OnBoard) { try { wait.until(ExpectedConditions
			 * .visibilityOfElementLocated(By.xpath("//*[@class=\"pull-left\"]/strong")));
			 * WebElement ONBOARD =
			 * Driver.findElement(By.xpath("//*[@class=\"pull-left\"]/strong")); if
			 * (ONBOARD.getText().equalsIgnoreCase("ON BOARD")) {
			 * logger.info("SD Job is on OnBoard stage");
			 * 
			 * } } catch (Exception stage) { String Orderstage =
			 * Driver.findElement(By.xpath("//strong/span[@class=\"ng-binding\"]"))
			 * .getText(); logger.info("Current stage of the order is=" + Orderstage);
			 * logger.info("Issue in Order stage==" + Orderstage); getScreenshot(Driver,
			 * "StageIssue_" + Orderstage);
			 * 
			 * } } }
			 */
		}
